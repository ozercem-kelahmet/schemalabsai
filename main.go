...
services.InitSpark()
	...
	// Initialize Kafka (fallback: Redis)
	services.InitKafka()
	services.InitKafkaConsumer()
	handlers.InitSSEKafkaCallback()
	...
	http.HandleFunc("/api/train/stream", enableCORS(handlers.AuthMiddleware(handlers.SSEHandler)))
	...
	http.HandleFunc("/api/predict/batch", enableCORS(handlers.AuthMiddleware(handlers.BatchPredictHandler)))
	...
	http.HandleFunc("/api/scheduler/sync", enableCORS(handlers.AuthMiddleware(handlers.TriggerSchedulerSyncHandler)))
