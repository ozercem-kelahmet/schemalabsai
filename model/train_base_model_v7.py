"""
SchemaLabs.AI Base Model v7
İlk N dataset backbone train, sonra freeze
"""
import warnings
...
def train_one_dataset(model, X, y, n_classes, n_features, device, train_backbone=True):
    ...
    if train_backbone:
        ...
        optimizer = AdamW(model.final_head.parameters(), lr=0.01, weight_decay=0.01)
    ...
    for epoch in range(max_epochs):
        ...
        print(f"    Ep {epoch+1}/{max_epochs} [{bb}]: train {train_acc:.1f}% | val {val_acc:.1f}% | best {best_val_acc:.1f}%{status}")
...
def train(data_dir="data/base_model", max_samples=1_000_000):
    print("=" * 60)
    print("SchemaLabs.AI Base Model v7")
    print("İlk 50 dataset backbone train, sonra freeze")
    print("=" * 60)
    ...
    # İlk 50 dataset backbone train, sonra freeze
    backbone_train_count = 50
    
    for i, meta in enumerate(selected_meta):
        train_backbone = (i < backbone_train_count)
        mode = "BACKBONE" if train_backbone else "FROZEN"
        
        print(f"\n[{i+1}/{len(selected_meta)}] [{mode}] {meta['filename']} - {meta['n_classes']} classes, {meta['n_rows']} rows")
        
        X, y = load_dataset(Path(data_dir) / meta["filename"])
        n_classes = meta["n_classes"]
        n_features = meta["n_features"]
        
        val_acc = train_one_dataset(model, X, y, n_classes, n_features, device, train_backbone=train_backbone)
        ...
        if i == backbone_train_count - 1:
            ...
            }, checkpoint_dir / "base_model_backbone.pt")
            print(f"\n*** Backbone training complete! Saved checkpoint ***\n")
