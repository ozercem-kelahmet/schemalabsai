...
from layers.cell_processing_sinusoidal import CellProcessing
...
class TabularFoundationModel(nn.Module):
    def __init__(self, config):
        ...
        # Feature AGNOSTIC - Attention Pooling
        # Her feature'ı d_model'e project et, sonra attention ile birleştir
    ...
    def forward(self, values, mask=None, schema_info=None, task='classification', cat_indices=None):
        ...
        batch_size, n_feat = values.shape
        
        # Her feature'ı ayrı ayrı project et
        v = values.unsqueeze(-1)  # (batch, n_feat, 1)
        v = self.value_proj(v)  # (batch, n_feat, d_model)
        v = self.value_norm(v)
        v = F.relu(v)
        
        # Self-attention ile birleştir
        q = self.attn_query(v.mean(dim=1, keepdim=True))  # (batch, 1, d_model)
        k = self.attn_key(v)  # (batch, n_feat, d_model)
        
        attn = torch.matmul(q, k.transpose(-2, -1)) / (self.d_model ** 0.5)  # (batch, 1, n_feat)
        attn = F.softmax(attn, dim=-1)
        
        values_emb = torch.matmul(attn, v).squeeze(1)  # (batch, d_model)
        values_emb = self.attn_out(values_emb)
    ...
    def update_heads(self, n_classes=None, n_features=None, n_sectors=None):
        ...
        if n_features:
            self.n_features = n_features
            # Feature AGNOSTIC - güncelleme gerekmiyor
