"""
SchemaLabs.AI Base Model v4
Her dataset için classifier reset, backbone paylaşımlı
"""
import warnings
...
class BaseModel83(nn.Module):
    def __init__(self, config):
        ...
        # Classifier dinamik oluşturulacak
    ...
    def set_classifier(self, n_classes):
        self.classifier = nn.Linear(self.d_model, n_classes).to(next(self.parameters()).device)
    ...
    def forward(self, x):
        ...
        logits = self.classifier(features) if self.classifier else None
...
def train(data_dir="data/base_model", max_samples=1_000_000):
    print("=" * 60)
    print("SchemaLabs.AI Base Model v4")
    print("Her dataset için classifier reset, backbone paylaşımlı")
    print("=" * 60)
    ...
    # Backbone optimizer (classifier hariç)
    backbone_params = [p for n, p in model.named_parameters() if 'classifier' not in n]
    optimizer = AdamW(backbone_params, lr=0.001, weight_decay=0.01)
    ...
    for epoch in range(epochs):
        ...
        for i, meta in enumerate(train_meta):
            ...
            # Bu dataset için classifier oluştur
            model.set_classifier(n_classes)
            
            # Classifier için ayrı optimizer
            cls_optimizer = AdamW(model.classifier.parameters(), lr=0.01)
            
            idx = np.random.permutation(len(X))
            X, y = X[idx], y[idx]
            
            # Bu dataset üzerinde birkaç step train
            for j in range(0, min(len(X), 2048), batch_size):
                ...
                with autocast('cuda'):
                    ...
                    loss = ce_loss + sector_loss + out["mcm_loss"]*0.1
                
                scaler.scale(loss).backward()
                scaler.unscale_(optimizer)
                scaler.unscale_(cls_optimizer)
        ...
        with torch.no_grad():
            for meta in val_meta:
                ...
                out = model(X_t)
                val_correct += (out["logits"].argmax(1) == y_t).sum().item()
                val_total += len(y_t)
