"""
SchemaLabs.AI Base Model v6
Backbone paylaşımlı, head her dataset için reset
Agnostic + Sector tahmin
"""
import warnings
...
def get_dynamic_config(n_samples, n_features, n_classes):
    if n_samples < 100:
        batch_size, epochs, lr = 4, 100, 0.005
    elif n_samples < 500:
        batch_size, epochs, lr = 16, 50, 0.005
    elif n_samples < 2000:
        batch_size, epochs, lr = 32, 30, 0.003
    else:
        batch_size, epochs, lr = 64, 20, 0.001
...
class BaseModel83(nn.Module):
    def __init__(self, config):
        ...
        # BACKBONE - paylaşımlı, tüm dataset'lerden öğrenir
        ...
        # HEAD - her dataset için reset edilir
        self.n_classes = 10
    ...
    def reset_head(self, n_classes):
        """Head'i sıfırdan oluştur - backbone korunur"""
        ...
        for m in self.final_head.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)
                if m.bias is not None: nn.init.zeros_(m.bias)
...
def train_one_dataset(model, X, y, n_classes, n_features, device, backbone_optimizer, scaler):
    """Tek dataset train - backbone paylaşımlı, head reset"""
    
    # Head reset
    model.reset_head(n_classes)
    
    # Head için ayrı optimizer
    head_optimizer = AdamW(model.final_head.parameters(), lr=0.01, weight_decay=0.01)
...
def train(data_dir="data/base_model", max_samples=1_000_000):
    print("=" * 60)
    print("SchemaLabs.AI Base Model v6")
    print("Backbone paylaşımlı + Head reset")
    print("Agnostic + Sector tahmin")
    print("=" * 60)
    ...
    config = {"max_features": max_features, "d_model": 256, "n_heads": 8, "n_layers": 3, "n_latents": 64, "n_sectors": 50}
    
    # TEK MODEL - tüm dataset'ler için
    model = BaseModel83(config).to(device)
    print(f"Parameters: {sum(p.numel() for p in model.parameters()):,}")
    
    # Backbone optimizer - paylaşımlı parametreler
    backbone_params = [p for n, p in model.named_parameters() if 'final_head' not in n]
    ...
    for i, meta in enumerate(selected_meta):
        ...
        val_acc = train_one_dataset(model, X, y, n_classes, n_features, device, backbone_optimizer, scaler)
