...
class GlobalReasoning(nn.Module):
    ...
    def forward(self, ctx):
        ...
        for l in self.layers: lat = l(lat, ctx)
...
def train():
    ...
    # Shard-sequential loading (no DataLoader — direct shard I/O)
    n_shards = meta["n_shards"]
    val_shards = max(1, n_shards // 10)
    train_shards = list(range(n_shards - val_shards))
    val_shard_ids = list(range(n_shards - val_shards, n_shards))
    tsz = len(train_shards) * cfg.shard_size
    vsz = val_shards * cfg.shard_size
    ...
    if torch.cuda.is_available() and hasattr(torch, 'compile'):
        try:
            ...
            log.info(f"  torch.compile failed ({e}), continuing without")
    ...
    for epoch in range(se2, cfg.total_epochs):
        ...
        for bi, batch in enumerate(shard_batches(shard_order, cfg.batch_size, shuffle=True)):
            if _shutdown:
        ...
        v_embeddings = []
        with torch.no_grad():
            for batch in shard_batches(val_shard_ids, cfg.batch_size, shuffle=False):
        ...
        ewc.decay_fisher()
        if epoch > 0:
            val_dl_tmp = list(shard_batches(val_shard_ids[:1], cfg.batch_size, shuffle=False))
        ...
        if cfg.self_learning_enabled and epoch > 0:
            ...
            with torch.no_grad():
                for batch in shard_batches(val_shard_ids[:1], cfg.batch_size, shuffle=False):
