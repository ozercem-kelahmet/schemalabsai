...
export function ConnectModal({ open, onOpenChange, onConnect }: ConnectModalProps) {
  ...
  const handleConnect = async (type: string) => {
    ...
    if (multiTableTypes.includes(connSubType)) {
      setIsLoadingTables(true)
      try {
        // Create connection first
        const createRes = await fetch("/api/connections/create", {
          method: "POST",
          credentials: "include",
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 text-sm">
          <button className="text-primary hover:underline" onClick={() => setSelectedTables(availableTables.map(t => t.name))}>Select All</button>
          <span className="text-muted-foreground">|</span>
          <button className="text-primary hover:underline" onClick={() => setSelectedTables([])}>Deselect All</button>
        </div>
      </div>
      <p className="text-sm text-muted-foreground">{selectedTables.length} of {availableTables.length} tables selected</p>
      <div className="max-h-[400px] overflow-y-auto space-y-1 border rounded-lg p-2">
        {availableTables.map((table) => (
          <label key={table.name} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 cursor-pointer">
            <input
              type="checkbox"
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <Button onClick={handleSaveSelectedTables} disabled={selectedTables.length === 0}>
          {saving ? "Saving..." : `Save (${selectedTables.length} tables)`}
        </Button>
      </div>
    </div>
  )
      ...
      <DialogContent className="border-border bg-card sm:max-w-[600px] flex flex-col overflow-hidden">
        ...
        {isLoadingTables ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <span className="text-sm text-muted-foreground">Connecting and fetching tables...</span>
          </div>
        ) : step === "tables" ? (
