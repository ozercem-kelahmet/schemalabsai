...
interface VConfig { id: string; name: string; description: string; config_yaml: string; enabled: boolean; version: number; model_id: string }
              ...
              <div key={v.id} className={cn("rounded-lg border transition-all", selectedId === v.id ? "border-foreground/20 bg-muted/40" : "border-transparent")}>
                ...
                {selectedId === v.id && (
                  <div className="px-3 pb-2 space-y-0.5 border-t border-border/50 pt-1">
                    ...
                    {tools[0] ? <Row label={`${tools[0].name}.py`} tag={tools[0].hook} valid={tools[0].validation_status} onView={() => setViewingCode({ name: tools[0].name, code: tools[0].code })} onEdit={() => openEdit("tool", tools[0], v.id)} onDelete={() => deleteItem("tool", tools[0].id, tools[0].name)} /> : <button className="text-[10px] text-muted-foreground hover:text-foreground transition-colors py-1.5 px-2" onClick={() => openUpload("tool", v.id)}>+ Add Tool</button>}
                    {agents[0] ? <Row label={`${agents[0].name}.py`} tag={agents[0].role} valid={agents[0].validation_status} onView={() => setViewingCode({ name: agents[0].name, code: agents[0].code })} onEdit={() => openEdit("agent", agents[0], v.id)} onDelete={() => deleteItem("agent", agents[0].id, agents[0].name)} /> : <button className="text-[10px] text-muted-foreground hover:text-foreground transition-colors py-1.5 px-2" onClick={() => openUpload("agent", v.id)}>+ Add Agent</button>}
                  </div>
                )}
              </div>
            )
