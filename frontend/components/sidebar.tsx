...
function SidebarInner() {
  ...
  const [displayEpochs, setDisplayEpochs] = useState(0)
  ...
  const [trainingStates, setTrainingStates] = useState<{[key: string]: TrainingState}>({})
  ...
  }, [trainingQueryId, queries, updateQuery, stopPolling])
  ...
  useEffect(() => {
    ...
    return () => stopPolling()
  }, [queries, startPolling, stopPolling])
  ...
  const handleDuplicateQuery = async (query: QueryItem) => {
    ...
    if (duplicated) {
      router.push("/playground/" + duplicated.id)
    }
  }
  ...
  const handleFineTune = async () => {
    console.log("=== handleFineTune START ===")
    console.log("selectedFiles:", selectedFiles)
    console.log("newProjectName:", newProjectName)
    if (selectedFiles.length === 0 || !newProjectName.trim()) {
      console.log("EARLY RETURN - no files or name")
      return
    }
    ...
    try {
      ...
      console.log("=== Calling api.multiTrain ===")
      ...
      setTrainingProgress(100)
      setTrainingComplete(true)
      setDisplayEpochs(result.epochs || smartEpochs)
      setTrainingEpoch(result.epochs || smartEpochs)
  ...
  const handleQueryClick = (query: QueryItem) => {
    // Always navigate to playground - training view will be shown there
            ...
            <SidebarMenu>
              <SidebarMenuItem>
                <Dialog open={isNewChatOpen} onOpenChange={(open) => { if (open) { resetDialog(); setExpandedFolders(["files"]); setIsNewChatOpen(true); } else { setIsNewChatOpen(false); } }}>
                  ...
                  <DialogContent className="sm:max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto" onInteractOutside={(e) => e.preventDefault()} onEscapeKeyDown={(e) => e.preventDefault()}>
                    <DialogHeader>
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="grid grid-cols-3 gap-3 text-center">
                                <div className="p-3 bg-secondary/50 rounded-lg">
                                  <div className="text-xl font-bold">{trainingEpoch}/{displayEpochs}</div>
                                  <div className="text-xs text-muted-foreground">Epoch</div>
                                </div>
                                <div className="p-3 bg-secondary/50 rounded-lg">
                                  <div className="text-xl font-bold">{(trainingLoss || 0).toFixed(3)}</div>
                                  <div className="text-xs text-muted-foreground">Loss</div>
                                </div>
                                <div className="p-3 bg-secondary/50 rounded-lg">
                                  <div className="text-xl font-bold text-green-600">{((trainingAccuracy || 0) * 100).toFixed(1)}%</div>
                                        ...
                                        >
                                          <div className="w-20">
                                            {selectedExistingModel === model.id && (
                                              <Badge className="bg-green-500/10 text-green-600 border-green-500/30 text-xs shrink-0">Selected</Badge>
                                            )}
                                          </div>
