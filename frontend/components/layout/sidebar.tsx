...
export function SidebarProvider({ children }: { children: React.ReactNode }) {
  ...
  const addChatSession = (session: PlaygroundSession) => {
    ...
    fetch("/api/queries/create", {
      ...
      credentials: "include",
      body: JSON.stringify({ name: session.name, model: "gpt-4o" }),
...
function ChatSessionItem({ session, theme }: { session: PlaygroundSession; theme: "dark" | "light" }) {
  ...
  if (isEditing) {
    return (
      <div className="flex items-center gap-1 pl-6 pr-2 py-1.5">
        <input ref={inputRef} type="text" value={editName} onChange={(e) => setEditName(e.target.value)} onKeyDown={handleKeyDown} onBlur={handleRename}
          className={cn("flex-1 text-xs px-1.5 py-1 rounded border bg-transparent outline-none min-w-0", theme === "dark" ? "border-white/20 text-white" : "border-gray-300 text-gray-900")} />
        <button onClick={handleRename} className={cn("p-1 rounded hover:bg-white/10", theme === "dark" ? "text-green-400" : "text-green-600")}><Check className="h-3 w-3" /></button>
        <button onClick={() => { setEditName(session.name); setIsEditing(false) }} className={cn("p-1 rounded hover:bg-white/10", theme === "dark" ? "text-gray-400" : "text-gray-600")}><X className="h-3 w-3" /></button>
      </div>
    )
  }
  ...
  return (
    <div className="group relative flex items-center">
      <Link href={`/playground/${session.id}`}
        className={cn("flex items-center rounded-lg pl-6 pr-2 py-2 text-xs transition-colors flex-1 min-w-0", theme === "dark" ? "text-gray-500 hover:bg-white/5 hover:text-gray-300" : "text-gray-500 hover:bg-gray-100 hover:text-gray-700")}
        title={session.name}>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-32">
          <DropdownMenuItem onClick={() => setIsEditing(true)} className="gap-2 text-xs"><Pencil className="h-3 w-3" />Rename</DropdownMenuItem>
          <DropdownMenuItem onClick={handleDelete} className="gap-2 text-xs text-red-500 focus:text-red-500"><Trash2 className="h-3 w-3" />Delete</DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
  )
...
export function Sidebar() {
  ...
  const handleNewChat = () => { router.push("/playground?new=" + Date.now()); setMobileOpen(false) }
    ...
    <>
      ...
      <button
        ...
        className={cn(
          ...
          theme === "dark" ? "border-white/10 bg-[#0A0A0B] text-white" : "border-gray-200 bg-white text-gray-900"
        )}
      ...
      >
        <div className={cn("flex h-16 items-center gap-2 border-b px-4", theme === "dark" ? "border-white/10" : "border-gray-200")}>
          {(!collapsed || mobileOpen) ? (
            ...
            <span className={cn("mx-auto text-lg font-semibold", theme === "dark" ? "text-white" : "text-gray-900")}>S</span>
          )}
        </div>
                    ...
                    {chatSessions.length === 0 ? (
                      ...
                      <>
                        ...
                        {chatSessions.length > 5 && (
                          <Link href="/playground?view=history" className={cn("flex items-center rounded-lg pl-6 pr-2 py-2 text-xs transition-colors", theme === "dark" ? "text-gray-500 hover:bg-white/5 hover:text-gray-300" : "text-gray-500 hover:bg-gray-100 hover:text-gray-700")}>
        ...
        <div className={cn("border-t px-3 py-4 space-y-1", theme === "dark" ? "border-white/10" : "border-gray-200")}>
          <button onClick={() => setCollapsed(!collapsed)} className={cn("hidden md:flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors w-full", theme === "dark" ? "text-gray-400 hover:bg-white/5 hover:text-white" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900", (collapsed && !mobileOpen) && "justify-center px-2")} title={(collapsed && !mobileOpen) ? "Expand" : "Collapse"}>
            {(collapsed && !mobileOpen) ? <PanelLeft className="h-5 w-5 shrink-0" /> : <><PanelLeftClose className="h-5 w-5 shrink-0" /><span>Collapse</span></>}
          </button>
          <button onClick={() => setHelpModalOpen(true)} className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors w-full", theme === "dark" ? "text-gray-400 hover:bg-white/5 hover:text-white" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900", (collapsed && !mobileOpen) && "justify-center px-2")} title={(collapsed && !mobileOpen) ? "Help" : undefined}>
            <HelpCircle className="h-5 w-5 shrink-0" />
            {(!collapsed || mobileOpen) && "Help"}
          </button>
          <button onClick={toggleTheme} className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors w-full", theme === "dark" ? "text-gray-400 hover:bg-white/5 hover:text-white" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900", (collapsed && !mobileOpen) && "justify-center px-2")} title={theme === "dark" ? "Switch to Light Mode" : "Switch to Dark Mode"}>
            {theme === "dark" ? <><Sun className="h-5 w-5 shrink-0" />{(!collapsed || mobileOpen) && <span>Light Mode</span>}</> : <><Moon className="h-5 w-5 shrink-0" />{(!collapsed || mobileOpen) && <span>Dark Mode</span>}</>}
          </button>
        </div>
